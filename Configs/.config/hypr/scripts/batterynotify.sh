...
"--execute"|"-e") execute=$2 ; shift 2 ;;
    ...
    *|"--help"|"-h")
      ...
      Visit https://github.com/itsivali/hypr-config for the Github Repo"
      exit 0
      ;;
  esac
done
is_laptop() { # Check if the system is a laptop
    ...
    echo "Cannot Detect a Battery. If this seems an error please report an issue to https://github.com/itsivali/hypr-config."
...
case "$battery_status" in         # Handle the power supply status
                ...
                # Due to modifications of some devices Not Charging after reaching 99 or limits
                    #if [[ ! -f "/tmp/hypr-config.batterynotify.status.$battery_status-$$" ]] && [[ "$battery_status" == "Not"* ]] ; then 
                    #touch "/tmp/hypr-config.batterynotify.status.$battery_status-$$"
                    #count=$(( timer > $mnt ? timer :  $mnt )) # reset count                    
                ...
                "Full") if $verbose; then echo "Case:$battery_status Level: $battery_percentage" ;fi
                    ...
                    ;;                                   
                    *)
                    if [[ ! -f "/tmp/hypr-config.batterynotify.status.fallback.$battery_status-$$" ]]; then 
                    echo "Status: '==>> "$battery_status" <<==' Script on Fallback mode,Unknown power supply status.Please copy this line and raise an issue to the Github Repo.Also run 'ls /tmp/hypr-config.batterynotify' to see the list of lock files.*"
                    touch "/tmp/hypr-config.batterynotify.status.fallback.$battery_status-$$"
                    fi     
                    fn_percentage 
                    ;;
            esac
...
main() { # Main function
    if is_laptop; then
rm -fr /tmp/hypr-config.batterynotify* # Cleaning the lock file
